...
- AGENTS.md is the canonical shared repository instruction file. `apps/gui-to-code/AGENTS.md` is the sole permitted scoped supplement for that independently shipped application; it must defer shared policy to this file.
...
- Do not duplicate repository instructions into a second long-lived source of truth. The permitted GUI-to-code supplement contains only local editor boundaries and points here for all shared policy.
...
- All visible strings, errors, empty states, shortcuts and accessibility labels are Russian. Keep the initial surface simple: one canvas, a small add menu, selection inspector, and live code/diagnostic panels; disclose advanced actions only on selection or request.
- Any semantic-edit RPC belongs in `packages/vite-plugin`; production editing requires an explicit authenticated write API. Vite HMR RPC is dev-only and is not a production persistence design.
