...
export function AppWP05() {
  ...
  const importSource = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      ...
      runtime.setCommandError(userError(
        userMessages.importFailed,
  ...
  return (
    <main className="editor-shell" onKeyDown={semantic.handleEditorKeyDown}>
      <header className="topbar">
        ...
        <div className="actions">
          ...
          <button type="button" onClick={() => downloadSource(runtime.source)}>
            Экспортировать model.c4
          </button>
          <button
            type="button"
            aria-label="Отменить последнее изменение"
            disabled={undoDisabled}
            onClick={() => void semantic.undo()}>
            Отменить
          </button>
          <button
            type="button"
            ...
            onClick={() => void semantic.redo()}>
            Повторить
          </button>
          <button
            type="button"
            disabled={runtime.busy}
            onClick={() => semantic.updateDraftSource(starterSource)}>
            Восстановить пример
          </button>
        </div>
      </header>
      ...
      <section className="workspace" aria-label="Рабочая область редактора LikeC4">
        <section className="panel structure-panel" aria-label="Структура модели">
          ...
          <StructureTree
            ...
            onSelect={id => semantic.selectElement(id)} />
        </section>
        ...
        <section className="panel diagram-panel" aria-label="Холст диаграммы" tabIndex={0}>
          <header>
            ...
            <div className="actions" aria-label="Инструменты диаграммы">
              {createKinds.map(([kind, label]) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={semantic.activeKind === kind}
                    disabled={semantic.canvasDisabled || unavailable}
                    title={unavailable ? 'Этот тип элемента недоступен в текущей спецификации.' : undefined}
                    onClick={() => semantic.activateCreateTool(kind)}>
                    {label}
                  </button>
                )
              })}
              <button
                ...
                aria-pressed={semantic.relationActive}
                disabled={semantic.canvasDisabled || semantic.elements.length < 2}
                onClick={semantic.activateRelationTool}>
                Связать
              </button>
            </div>
          </header>
              ...
              <LikeC4EditorProvider editor={runtime.editor}>
                <LikeC4ModelProvider likec4model={runtime.renderModel}>
                  <ReactLikeC4
                    ...
                    onNodeClick={node => {
                      if (node.modelRef) semantic.selectElement(node.modelRef as Fqn, false)
                    }}
                    onConnect={semantic.relationActive
                      ? (sourceId, targetId) => semantic.completeRelation(sourceId, targetId)
                      : null}
                    onCanvasClick={event => {
                      if (!semantic.activeKind) return
                      semantic.controller.current?.requestElementCreation({
                        x: event.clientX,
                        y: event.clientY,
                      })
                    }} />
          ...
          {semantic.activeKind && (
            <p aria-live="polite">Кликните по холсту или нажмите Enter, чтобы создать элемент.</p>
          )}

          {semantic.relationActive && (
            <section className="relation-controls" aria-label="Создание связи с клавиатуры">
              <p aria-live="polite">
                Перетащите маркер исходного элемента на целевой или выберите элементы ниже.
              </p>
              <label>
                Исходный элемент
                <select
                  aria-label="Исходный элемент связи"
                  value={semantic.relationSource}
                  onChange={event => {
                    semantic.setRelationSource(event.target.value)
              ...
              <label>
                ...
                <select
                  ...
                  value={semantic.relationTarget}
                  onChange={event => semantic.setRelationTarget(event.target.value)}>
              ...
              <button
                type="button"
                onClick={() => semantic.completeRelation(
                  semantic.relationSource,
                  semantic.relationTarget,
                )}>
                Создать связь
              </button>
            </section>
          ...
          {runtime.persistenceNotice && (
            <p role="status" aria-live="polite">{runtime.persistenceNotice}</p>
          )}
