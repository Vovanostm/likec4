...
export class ElementViewDocumentEditService {
  ...
  async planAddElementView(input: AddElementViewInput): Promise<SourceEditPlan> {
    ...
    const document = input.documentUri
      ? this.langium.shared.workspace.LangiumDocuments.getOrCreateDocument(sourceUri(input.documentUri))
      : scope.document
    if (!document) {
...
function sourceUri(value: string): URI {
  if (value.includes(':')) return URI.parse(value)
  const path = value.startsWith('/') ? value.slice(1) : value
  return URI.parse(`virtual:/workspace/${path}`)
}
